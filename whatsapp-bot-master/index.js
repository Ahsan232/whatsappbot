...
const start = (aruga = new Client()) => {
    ...
    console.log(color(figlet.textSync('AHSANJAY BOT 2.0', { font: 'Ghost', horizontalLayout: 'default' })))
    console.log(color(figlet.textSync('----------------', { horizontalLayout: 'default' })))
    console.log(color('[DEV]'), color('Aruga', 'yellow'))
	...
	// kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
        ...
        await aruga.simulateTyping(chat.id, true).then(async () => {
          await aruga.sendText(chat.id, `Hai minna~, Im AhsanJay BOT 2.0. To find out the commands on this bot type ${prefix}menu`)
        })
	    }
	}
    })
    ...
    aruga.onGlobalParicipantsChanged(async (event) => {
        ...
        if (event.action === 'add' || event.action === 'invite') await aruga.sendTextWithMentions(event.chat, `YO BROO, *Selamat bergabung ke grup ini ;)*, jangan lupa lihat Deskripsi @${event.who.replace('@c.us', '')} \n\nHave fun with us✨`)

        // kondisi ketika seseorang dikick/keluar dari group
	    if (event.action === 'remove' || event.action === 'leave') await aruga.sendTextWithMentions(event.chat, `Telah meninggal kan Grup, YTH @${event.who.replace('@c.us', '')}, We'll miss you`)
    })

    aruga.onIncomingCall(async (callData) => {
        // ketika seseorang menelpon nomor bot akan mengirim pesan
        await aruga.sendText(callData.peerJid, '*Jangan menelpon, Atau akan di blokir secara otomatis oleh BOT*.\n\n-bot')
        .then(async () => {
    ...
    try {
        ...
        case 'menuadmin':
            ...
            if (!isGroupAdmins) return aruga.reply(from, 'Perintah gagal, *perintah hanya bisa dipakai ADMIN*', id)
            ...
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await aruga.sendText(chekgrup.id, `Hai Semua, aq AhsanJay BOT 2.0. To find out the commands on this bot type ${prefix}menu`)
                      })
            } else {
              ...
              try {
	   	    ...
	   	    await aruga.reply(from, 'maaf batas penggunaan hari ini sudah maksimal', id)
                ...
                aruga.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ...
                    : aruga.reply(from, 'NIH, STIKER LU')).then(() => console.log(`Sticker Processed for ${processTime(t, moment())} Second`))
            } else {
        ...
        case 'stickergiphy':
	 {
        ...
        case 'nulis':
            ...
            await aruga.sendImage(from, `${nulisp}`, '', 'NIH!!, MAMAM', id)
            break
        ...
        case 'kpop':
            ...
            if (args[0] == 'blackpink' || args[0] == 'exo' || args[0] == 'bts') {
                ...
                .then(body => {
                    ...
                    aruga.sendFileFromUrl(from, randomkpopx, '', 'EAKK, KPOPERS YAA... :>', id)
                })
            } else {
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *A H S A N J A Y* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *A H S A N J A Y* 〙\n\n${msg}`)
            }
